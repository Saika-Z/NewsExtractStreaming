...
object KafkaUtil {
  /**
    * 得到kafka中指定topic下每个partitioner最早存在offset
    */
  def getPartitionOffset(topic: String):mutable.HashMap[Long,Long] = {
    val topics:Set[String] = Set(topic)
    //存储kafka集群中每个partition当前最早的offset
    var clusterEarliestOffsets = mutable.HashMap[Long,Long]()
    val consumer:SimpleConsumer = new SimpleConsumer(host = "s1.hadoop",port = 9092,soTimeout = 10000,bufferSize = 64*1024,
      clientId ="leaderLookup" + System.currentTimeMillis())
    //使用隐式转化进行java和scala类型的相互转换
    ...
    metadatas.map(f => {
      ...
      var requestInfo = new mutable.HashMap[TopicAndPartition, PartitionOffsetRequestInfo]();
  ...
  /**
    ...
    */
  def getPartitionOffsetZK(topic:String, zkTopicPath:String,clusterEarliestOffsets:HashMap[Long,Long], zkClient:ZkClient):HashMap[TopicPartition,Long]= {
    var nowOffset = new HashMap[TopicPartition, Long]

    for ((clusterPartition, clusterEarliestOffset) <- clusterEarliestOffsets) {
      //zkTopicPath = /consumer/qingniu/offset/hainiu_qingniu
      //判断 /consumer/${group}/offsets/${topic}/${partitionId}/ 是否存在
      val zkExist: Boolean = zkClient.exists(s"$zkTopicPath/${clusterPartition}")
      val tp = new TopicPartition(topic, clusterPartition.toInt)
      if (zkExist) {
        //从zk中查询该partition的offset，这个offset是我们自己根据每个topic的不同partition
        //数据路径例子：/consumer/${group}/offsets/${topic}/${partitionId}
        val partitionOffset = zkClient.readData[String](s"$zkTopicPath/${clusterPartition}")
        // hainiu_qingniu/0
        val myOffset = partitionOffset.toLong

        //将每个partition对应的offset保存到nowOffsets中，根据zk中保存的历史偏移量与kafka
